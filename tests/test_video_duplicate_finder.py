...
def test_same_size_and_similar_name_are_strong_candidates() -> None:
    reason = candidate_reason(info("holiday.mp4", 1000), info("holiday (1).mp4", 1000), 80)
...
def test_different_size_and_similar_name_are_warning_candidates() -> None:
    reason = candidate_reason(info("holiday.mp4", 1000), info("holiday (1).mp4", 2000), 80)
    assert reason is not None
...
def test_grouping_uses_connected_components() -> None:
    # B bridges A and C. A-C itself is below the 80% threshold.
    groups = find_duplicate_groups([
        info("movie 2026 final.mp4", 1000),
        info("movie 2026 final cut.mp4", 900),
        info("movie 2026 final extended.mp4", 800),
    ], 80)
    assert len(groups) == 1
    assert {item.path.name for item in groups[0]} == {
        "movie 2026 final.mp4",
        "movie 2026 final cut.mp4",
        "movie 2026 final extended.mp4",
    }
...
def test_same_name_different_sizes_are_not_called_exact_duplicates() -> None:
    reason = candidate_reason(info("lecture.mp4", 1000), info("lecture (1).mp4", 3000), 80)
    assert reason is not None
    assert "주의 후보" in reason
    assert "크기 다름" in reason
